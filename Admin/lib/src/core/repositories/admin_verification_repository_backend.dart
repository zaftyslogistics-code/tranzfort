...
class SupabaseAdminVerificationBackend implements AdminVerificationBackend {
  ...
  Future<List<Map<String, dynamic>>> fetchVerificationCases() async {
    ...
    try {
      final rows = await activeClient
          ...
          .select('id, subject_type, subject_id, case_status, assigned_admin_user_id, submitted_at, last_reviewed_at, current_decision_summary')
  ...
  Future<Map<String, dynamic>?> fetchVerificationCaseById(String caseId) async {
    ...
    try {
      final row = await activeClient
          .from('verification_cases')
          .select('id, subject_type, subject_id, case_status, assigned_admin_user_id, submitted_at, last_reviewed_at, current_decision_summary, current_review_feedback_json')
          .eq('id', caseId)
          .maybeSingle();
      if (row == null) {
        return null;
  ...
  Future<Map<String, dynamic>?> fetchProfileById(String id) async {
    ...
    try {
      final row = await activeClient
          .from('profiles')
          .select('id, full_name, mobile, email, user_role_type, verification_status, created_at, aadhaar_number, aadhaar_last4, pan_number, aadhaar_front_document_path, aadhaar_back_document_path, pan_document_path, profile_photo_document_path, verification_feedback_json')
          .eq('id', id)
          .maybeSingle();
      if (row == null) {
        return null;
