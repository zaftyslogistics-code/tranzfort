...
class _PagedTruckerMarketplaceBackend implements TruckerMarketplaceBackend {
  ...
  Future<MarketplaceSearchResult> searchLoads(
    ...
    final items = rows.map((row) => MarketplaceLoadItem(
      ...
      requiredVehicleCategoryCode: row['required_vehicle_category_code'] as String?,
      requiredBodyStyleCodes: (row['required_body_style_codes'] as List<dynamic>?)?.cast<String>() ?? const <String>[],
      requiredConfigurationCodes: (row['required_configuration_codes'] as List<dynamic>?)?.cast<String>() ?? const <String>[],
