...
class ChatRepository {
  ...
  Future<Result<List<Map<String, dynamic>>>> getConversations() async {
    try {
      ...
      if (user == null) {
        return const Failure(AppFailureType.auth, debugMessage: 'Not logged in');
      }
  ...
  Future<Result<List<Map<String, dynamic>>>> getMessages(String conversationId) async {
    try {
      final response = await _supabase
          .from('messages')
          .select()
  ...
  Future<Result<void>> markMessagesAsRead(String conversationId, String currentUserId) async {
    try {
      await _supabase
          .from('messages')
