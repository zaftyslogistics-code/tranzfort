...
class EntityExtractor {
  static const _materials = [
    'coal', 'steel', 'cement', 'iron ore', 'sand', 
    'grain', 'fertilizer', 'timber'
  ];

  static BotIntent determineIntent(String text) {
    final lower = text.toLowerCase();
    
    if (lower.contains('load dhundho') || lower.contains('find load') || lower.contains('load chahiye')) {
      return BotIntent.findLoad;
    }
    if (lower.contains('load dalna hai') || lower.contains('post load') || lower.contains('naya load')) {
      return BotIntent.postLoad;
    }
    if (lower.contains('meri loads') || lower.contains('my loads') || lower.contains('posted loads')) {
      return BotIntent.myLoads;
    }
    if (lower.contains('meri trips') || lower.contains('my trips') || lower.contains('trip status')) {
      return BotIntent.myTrips;
    }
    if (lower.contains('booking ka status') || lower.contains('check status') || lower.contains('kya hua')) {
  ...
  static double? extractWeight(String text) {
    final regex = RegExp(r'(\d+(?:\.\d+)?)\s*(ton|tonne|t)', caseSensitive: false);
  ...
  static double? extractPrice(String text) {
    final lakhRegex = RegExp(r'(\d+(?:\.\d+)?)\s*(lakh|l)', caseSensitive: false);
    ...
    final kRegex = RegExp(r'(\d+(?:\.\d+)?)\s*(k|thousand|hazar)', caseSensitive: false);
    ...
    final exactRegex = RegExp(r'(?:rs|rupees|₹)?\s*(\d{3,})', caseSensitive: false);
