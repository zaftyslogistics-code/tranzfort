...
void main() {
  group('Legacy auth provider tests (aligned)', () {
    test('AuthEntryState copyWith clears error when requested', () {
      const state = AuthEntryState(lastError: AppFailureType.network);
      final cleared = state.copyWith(clearError: true);
      expect(cleared.lastError, isNull);
    });

    test('AuthEntryNotifier sendOtp propagates failure state', () async {
    ...
    test('OtpVerificationNotifier verifyOtp success clears error', () async {
      final repository = MockAuthRepository();
    ...
    test('RoleSetupNotifier returns auth failure without active session', () async {
