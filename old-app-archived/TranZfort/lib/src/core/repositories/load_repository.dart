...
class LoadRepository {
  ...
  Future<Result<List<Map<String, dynamic>>>> findLoads({
    ...
    try {
      ...
      final response = switch (sortBy) {
        ...
        'pickup_date' =>
          await query.order('pickup_date', ascending: true).range(from, to),
        _ => await query.order('created_at', ascending: false).range(from, to),
      };
  ...
  Future<Result<List<Map<String, dynamic>>>> getSavedSearches(
    String userId,
  ) async {
    try {
      final response = await _supabase
  ...
  Future<Result<void>> saveSearch({
    ...
    try {
      await _supabase.from('user_saved_searches').insert({
        'user_id': userId,
        'origin_city': originCity,
        'destination_city': destinationCity,
        'material': material,
        'truck_type': truckType.isEmpty ? null : truckType,
        'sort_by': sortBy,
      });
      return const Success(null);
    } catch (e) {
  ...
  Future<Result<void>> deleteSavedSearch(String savedSearchId) async {
    try {
      await _supabase.from('user_saved_searches').delete().eq('id', savedSearchId);
      return const Success(null);
    } catch (e) {
      return Failure(classifyError(e), debugMessage: e.toString());
    }
  ...
  Future<Result<double>> getDieselPrice(String state) async {
    try {
      ...
      final value = CoordinateUtils.parseDouble(response['price_per_litre']) ?? 90;
  ...
  Future<Result<void>> startTrip({
    ...
    try {
      final parsed = CoordinateUtils.parseLatLng(lat, lng);
      final response = await _supabase.rpc(
        'start_trip',
        params: {
          'p_trip_id': tripId,
  ...
  Future<Result<void>> markDelivered({
    ...
    try {
      ...
      await _supabase
          ...
          .update({
            'stage': 'delivered',
            'last_known_lat': parsed?.lat,
            'last_known_lng': parsed?.lng,
