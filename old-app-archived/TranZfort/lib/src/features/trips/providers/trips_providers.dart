...
final tripRoutesServiceProvider = Provider<GoogleRoutesService>((ref) {
  final mapsConfig = ref.watch(mapsConfigProvider);
  return GoogleRoutesService(mapsConfig, OsrmService());
});
...
class TripActionNotifier extends StateNotifier<AsyncValue<void>> {
  final Ref _ref;
  String? _lastCapturedLocationLabel;

  TripActionNotifier(this._ref) : super(const AsyncData(null));

  String? get lastCapturedLocationLabel => _lastCapturedLocationLabel;
  ...
  Future<bool> startTrip(String tripId) async {
    ...
    final capturedLocationLabel = await _resolveCapturedLocationLabel(captured);
    ...
    switch (result) {
      case Success():
        _lastCapturedLocationLabel = capturedLocationLabel;
      ...
      case Failure(debugMessage: final msg):
        _lastCapturedLocationLabel = null;
  ...
  Future<bool> uploadLr({required String tripId, required File lrFile}) async {
    ...
    final uploadResult = await _ref
        .read(tripStorageServiceProvider)
        .uploadFileAtPath(
  ...
  Future<String?> _resolveCapturedLocationLabel(CapturedLocation? captured) async {
    if (captured == null) {
      return null;
    }

    final geocoded = await _ref
        .read(tripRoutesServiceProvider)
        .reverseGeocode(captured.lat, captured.lng);
    return geocoded?.cityStateLabel;
  }
}
