...
**Tasks:**
- [ ] **13.3.1** Add a small status row below outgoing message timestamp: `✓` (sent) / `✓✓` grey (delivered) / `✓✓` teal (read)
- [ ] **13.3.2** Use `AppColors.textMuted` for sent, `AppColors.primary` for read
- [ ] **13.3.3** Hide receipt row on incoming messages (only show for `isFromCurrentUser: true`)
- [ ] **13.3.4** If `isSending` (pending), show a small spinner instead of checkmark
...
**Tasks:**
- [ ] **13.4.1** Hide timestamp on a bubble if the **next** message is from the **same sender** within 2 minutes
- [ ] **13.4.2** Show a centered date pill (`Today`, `Yesterday`, `12 Apr`) when the calendar day changes between messages
- [ ] **13.4.3** Date pill style: `Container` with `AppColors.surfaceSoft`, `AppRadius.chip` (20px), `AppColors.textMuted` text, `AppSpacing.md` horizontal + `AppSpacing.xs` vertical padding
- [ ] **13.4.4** Modify `_buildRenderedMessages` in `chat_screen_action_extensions.dart` to compute grouping metadata (same-sender-within-2min flag, day-change flag)
- [ ] **13.4.5** Update `_ChatMessageBubble` constructor to accept `showTimestamp`, `showDateDivider` booleans
...
**Tasks:**
- [ ] **13.5.1** Only auto-scroll if `scrollController.position.pixels` is within 100px of `maxScrollExtent`
- [ ] **13.5.2** If scrolled up (> 100px from bottom), show a floating "New message" pill at bottom of chat area instead of jumping
- [ ] **13.5.3** New message pill: tap to scroll to bottom, auto-dismiss on tap or after 3 seconds
- [ ] **13.5.4** Pill style: `AppColors.inkSurface` background, `AppColors.inkTextPrimary` text, `AppRadius.chip`, `AppShadows.elevation2`
- [ ] **13.5.5** Modify `_scrollToBottom()` in `chat_screen_action_extensions.dart` with the threshold check
...
- [ ] No backend/provider/repository changes
- [ ] No new imports beyond existing `AppColors`, `AppSpacing`, `AppRadius`, `AppShadows`
- [ ] `AppTextField` constructor unchanged — only `_ChatComposer` swaps to local decorated `TextField`
- [ ] `_ChatContextBanner` constructor params unchanged — same callbacks
- [ ] All theme tokens from existing design system (no raw hex codes)
- [ ] Test voice recording still works (composer changes)
- [ ] Test booking Approve/Reject still works (banner changes)
- [ ] Test keyboard open/close still works (`AnimatedPadding` for `bottomInset`)
- [ ] Run `flutter analyze` before commit — zero new errors
...
**Tasks:**
- [x] **13.1.1** Remove `DetailSectionCard` wrapper; use compact `Material` card with `AppSpacing.md` padding and `AppColors.surfaceSoft` background
- [x] **13.1.2** Collapsed: single row — small route icon + truncated `routeLabel` + ONE `StatusChip` (booking status if present, else load status) + expand chevron
- [x] **13.1.3** Expanded: material, price, pickup date on a single `Wrap` row (not vertical stack) to save vertical space; hide redundant "Active" load status
- [x] **13.1.4** Approve/Reject row: ONLY render when `canShowBookingActions == true`; use compact `Row` with equal-width buttons, NOT a full `DetailSectionCard` footer
- [x] **13.1.5** Reduce top padding around banner from `AppSpacing.lg` to `AppSpacing.md` in `chat_screen.dart` line 284
- [x] **13.1.6** Trucker view: expanded banner shows identical load info but NEVER shows Approve/Reject; verify trucker `truckerChatBlocked` gating warning still renders below banner (separate from banner)
- [x] **13.1.7** Ensure `loadDetailState?.actionFailure` WarningBlock still renders between banner and messages for BOTH roles
...
**Tasks:**
- [x] **13.2.1** Replace `AppTextField` with a locally decorated `TextField` inside a `Container` with:
  - `BoxDecoration` using `AppColors.surfaceSoft` background, `BorderRadius.circular(AppRadius.input)` (12px), subtle `Border.all(color: AppColors.divider)`
  - Remove `Material` elevation 8; use `BoxShadow` from `AppShadows.elevation1` (soft 2px shadow)
- [x] **13.2.2** Wrap mic/send button area in `AnimatedSwitcher` with `TransitionBuilder` (rotation + fade, 180ms `Curves.easeInOut`)
- [x] **13.2.3** Send button: circular `Container` (48x48) with `AppColors.heroCta` gradient (teal→orange), white icon, no text label
- [x] **13.2.4** Recording state: red pulse dot (animated `Container` with `BoxShape.circle`, `Tween` between 8px→12px) inside the input area + elapsed time badge in `AppColors.error` color
- [x] **13.2.5** Keep `ValueListenableBuilder` for text → mic/send toggle logic unchanged
- [x] **13.2.6** Test on keyboard open/close (`AnimatedPadding` for `bottomInset` already exists — verify still works)
...
**Tasks:**
- [x] **13.6.1** Supplier group card: tighten padding from `AppSpacing.lg` to `AppSpacing.md`; reduce visual weight of group header
- [x] **13.6.2** Supplier group card footer: `_SupplierConversationRow` already uses `Ink` with border — keep it, but reduce inner padding to `AppSpacing.sm`
- [x] **13.6.3** Trucker card: unify with supplier row style — use same `InkWell` + `Border.all` pattern for consistency
- [x] **13.6.4** Both inboxes: add subtle `Divider` between last message preview and timestamp to improve scannability
- [x] **13.6.5** Do NOT add grouping to trucker inbox — keep flat list (truckers typically have fewer concurrent conversations)
