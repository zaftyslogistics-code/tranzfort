-- Fix get_public_load_previews: add explicit enum cast for status comparison
-- l.status is load_status enum, p_status_filter is TEXT — implicit cast can fail
-- in some PostgreSQL configurations. Explicit cast ensures reliable comparison.
...
COMMENT ON FUNCTION public.get_public_load_previews IS
'Returns paginated public-visible loads for a supplier profile. Only verified suppliers'' loads with status in (active, completed, assigned_partial, assigned_full) are visible.';
