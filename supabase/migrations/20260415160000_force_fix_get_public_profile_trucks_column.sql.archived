-- Force fix get_public_profile RPC - ensure trucks table uses verification_status
-- This migration drops and recreates the function to ensure the fix is applied
...
BEGIN
  ...
  if v_role = 'trucker' then
    -- Get fleet for trucker - trucks table has verification_status, NOT status
    select jsonb_agg(
        jsonb_build_object(
            'id', t.id,
