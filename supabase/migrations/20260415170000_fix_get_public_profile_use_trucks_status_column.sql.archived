-- Fix get_public_profile RPC - trucks table has 'status' column, not 'verification_status'
-- The trucks table schema shows it has 'status' (USER-DEFINED type), not 'verification_status'

DROP FUNCTION IF EXISTS get_public_profile(UUID, UUID);

CREATE FUNCTION get_public_profile(
    p_user_id uuid,
...
BEGIN
  ...
  if v_role = 'trucker' then
    -- Get fleet for trucker - trucks table has 'status' column, not 'verification_status'
    select jsonb_agg(
        jsonb_build_object(
            'id', t.id,
