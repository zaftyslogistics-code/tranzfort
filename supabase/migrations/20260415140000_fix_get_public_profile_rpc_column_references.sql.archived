-- Fix get_public_profile RPC column references
-- Issue 1: trips.status should be trips.stage (line 422)
-- Issue 2: trucks.status should be trucks.verification_status (line 438)
...
BEGIN
    ...
    -- Get completed trips count (FIXED: use stage instead of status)
    select count(*) into v_trips_count
    from trips
    where trucker_id = p_user_id and stage = 'completed';
