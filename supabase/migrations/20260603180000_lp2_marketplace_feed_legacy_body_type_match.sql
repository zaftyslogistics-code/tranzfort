-- Pinned Find Loads uses legacy required_body_type; match case-insensitively and
-- include catalog-tagged loads when body type aligns (open <-> open_truck).
BEGIN;
...
DECLARE
  ...
  v_total BIGINT;
  v_results JSONB;
  v_body_type TEXT := NULLIF(LOWER(TRIM(COALESCE(p_body_type, ''))), '');
BEGIN
  ...
  WHERE public.load_is_on_marketplace(l)
    ...
    AND (
      v_body_type IS NULL
      OR LOWER(TRIM(COALESCE(l.required_body_type, ''))) = v_body_type
      OR (
        v_body_type = 'open'
        AND LOWER(TRIM(COALESCE(l.required_vehicle_category_code, ''))) = 'open_truck'
      )
      OR (
        v_body_type = 'refrigerated'
        AND LOWER(TRIM(COALESCE(l.required_vehicle_category_code, ''))) = 'reefer'
      )
    )
    ...
    AND (
      ...
      OR LOWER(TRIM(COALESCE(l.required_vehicle_category_code, ''))) = LOWER(TRIM(p_required_vehicle_category_code))
      OR (
        LOWER(TRIM(COALESCE(l.required_vehicle_category_code, ''))) = ''
        AND v_body_type IS NOT NULL
        AND LOWER(TRIM(COALESCE(l.required_body_type, ''))) = v_body_type
      )
    )
    AND (
