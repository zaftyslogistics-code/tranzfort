-- Fix get_supplier_dashboard_stats RPC to join booking_requests with loads
-- booking_requests table doesn't have supplier_id column, need to join via loads
...
begin
    ...
    -- Pending bookings (booking_requests with status 'submitted')
    -- booking_requests doesn't have supplier_id, need to join with loads
    -- Valid booking_status values: 'submitted', 'approved', 'rejected', 'withdrawn', 'superseded'
