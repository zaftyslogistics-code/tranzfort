-- Prefer private storage paths over stale HTTP values in profiles.avatar_url (P0-7/P0-8).

CREATE OR REPLACE FUNCTION public.resolve_profile_avatar_source(
  p_avatar_url text,
...
AS $$
  SELECT COALESCE(
    CASE
      WHEN NULLIF(BTRIM(p_profile_photo_path), '') IS NOT NULL
        AND p_profile_photo_path NOT LIKE 'http://%'
        AND p_profile_photo_path NOT LIKE 'https://%'
      THEN BTRIM(p_profile_photo_path)
    END,
    CASE
      WHEN NULLIF(BTRIM(p_avatar_url), '') IS NOT NULL
        AND p_avatar_url NOT LIKE 'http://%'
        AND p_avatar_url NOT LIKE 'https://%'
        AND POSITION('/' IN BTRIM(p_avatar_url)) > 0
      THEN BTRIM(p_avatar_url)
    END,
    NULLIF(BTRIM(p_profile_photo_path), ''),
    CASE
      WHEN NULLIF(BTRIM(p_avatar_url), '') LIKE 'http://%'
        OR NULLIF(BTRIM(p_avatar_url), '') LIKE 'https://%'
      THEN BTRIM(p_avatar_url)
    END
  );
$$;

COMMENT ON FUNCTION public.resolve_profile_avatar_source(text, text) IS
  'Returns profile photo storage path or legacy HTTP URL; prefers profile_photo_document_path over stale avatar_url.';

-- Chat inbox: resolve avatars at query time.
DROP FUNCTION IF EXISTS public.get_current_user_conversation_summaries();
...
BEGIN
  ...
  SELECT
    ...
    public.resolve_profile_avatar_source(
      supplier_profile.avatar_url,
      supplier_profile.profile_photo_document_path
    ) AS supplier_avatar_url,
    ...
    public.resolve_profile_avatar_source(
      trucker_profile.avatar_url,
      trucker_profile.profile_photo_document_path
    ) AS trucker_avatar_url,
...
GRANT EXECUTE ON FUNCTION public.get_current_user_conversation_summaries() TO authenticated;
