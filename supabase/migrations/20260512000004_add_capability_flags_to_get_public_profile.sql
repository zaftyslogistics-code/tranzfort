-- Add capability flags to get_public_profile RPC
-- Part of Phase 9.1: Ensure capability flags are backend-authoritative
-- These flags determine UI permissions based on viewer-profile relationship
...
BEGIN
    ...
    IF p_viewer_id IS NOT NULL AND NOT v_is_self THEN
        -- Check if there's any business relationship (completed trips, active bookings, etc.)
        SELECT EXISTS (
            SELECT 1 FROM public.trips t
            WHERE t.trucker_id = p_user_id AND t.supplier_id = p_viewer_id
               AND t.stage IN ('in_progress', 'completed')
            UNION
            SELECT 1 FROM public.trips t
    ...
    v_can_review := v_has_business_relationship; -- Can only review if there's a business relationship
