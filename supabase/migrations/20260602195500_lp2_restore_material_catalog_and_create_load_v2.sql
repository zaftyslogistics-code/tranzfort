-- LOADPOST-2 restore after rollback migration.
-- Ensures material catalog + search RPC + create_load signature match current app payload.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'loads_material_code_fkey'
  ) THEN
    ALTER TABLE public.loads
